...
use inc\admin_views\WF_General_Settings_View;
use inc\admin_views\WF_Automation_Settings_View;
use inc\admin_views\WF_Accounting_Settings_View;
use inc\admin_views\WF_Order_Settings_View;
use inc\admin_views\WF_Product_Settings_View;
use inc\admin_views\WF_Shipping_Settings_View;
use inc\admin_views\WF_Bulk_Settings_View;
use inc\admin_views\WF_Advanced_Settings_View;
use inc\admin_views\WF_Upgrades_Settings_View;
use inc\fortnox\api\WF_Orders;
use inc\fortnox\api\WF_Products;
use Exception;
...
class WF_Plugin {
	...
	public static function add_settings(){
		...
		WF_Upgrades_Settings_View::add_settings();
	}
	...
	public static function render_popup_for_fortnox_sync_orders_date_range(){
		...
		include FORTNOX_PLUGIN_PATH . "/inc/templates/template-popup-fortnox-order-sync.php";
		include FORTNOX_PLUGIN_PATH . "/inc/templates/template-popup-fortnox-product-sync.php";
		?></section><?php
	}
	...
	public static function add_admin_scripts(){
		...
		wp_enqueue_script( 'fortnox', self::get_url( 'assets/js/admin.js'  ), [ 'jquery', 'jquery-ui-datepicker', 'selectWoo', 'select2' ] );
		...
		wp_enqueue_style( 'fortnox', self::get_url( 'assets/css/fortnox-admin.css'  ) );
	}

	public static function add_scripts_for_my_account(){
		wp_enqueue_script( 'my-account', self::get_url( 'assets/js/my-account.js'  ), array(), false, true );
		wp_enqueue_style( 'my-account', self::get_url( 'assets/css/my-account.css'  ) );
	}

	public static function add_styles_to_products_edit_view() {
		global $pagenow;
		$screen = get_current_screen();

		// Only for product edit page
		if( $pagenow == 'edit.php' && $screen->post_type == 'product' ) {
			wp_enqueue_style( 'fortnox-edit-products', self::get_url( 'assets/css/edit-products.css'  ) );
		}
	}
