...
namespace inc\fortnox;

if ( !defined( 'ABSPATH' ) ) die();

use Exception;
use inc\admin_views\WF_Admin_Listing_Actions;
use inc\fortnox\api\WF_Accounts;
use inc\fortnox\api\WF_Auth;
use inc\fortnox\api\WF_Delivery_Terms;
use inc\wetail\WF_Credentials;
use inc\wetail\WF_NG_Fortnox_auth;
...
class WF_Ajax {
    ...
    public static function init(){
	    ...
	    add_action( 'wp_ajax_fortnox_logout', __CLASS__ . '::logout_fortnox' );
        add_action( 'wp_ajax_fortnox_fetch_accounts', __CLASS__ . '::fetch_accounts' );
	...
	public static function fortnox_organization_number() {
		...
		if ( $auth_response = WF_NG_Fortnox_auth::request_for_auth($_REQUEST['key'], $_SERVER['HTTP_REFERER']) ) {
			self::respond( [
				'message' => sprintf(__( 'Organization number registered, next step is to register app in Fortnox. See our <a href="%s">installation guide</a> for further instructions.' , WF_Plugin::TEXTDOMAIN ), 'https://docs.wetail.io/woocommerce/fortnox-integration/fortnox-installationsguide/#aktivera-app'),
	...
	/** Fetches active accounts from Fortnox
     *
     */
	public static function fetch_accounts(){
		try {
			WF_Accounts::get_and_store_accounts_from_fortnox();
			self::respond( [ 'message' => __( "Accounts fetched.", WF_Plugin::TEXTDOMAIN ) ] );
		}
	...
	/**
	 * Deletes all fortnox account related settings
	 * @static 4.5
	 */
	public static function logout_fortnox(){

		$fortnox_keys = [
		 'fortnox_predefined_accounts','fortnox_access_token','fortnox_connected_organization_number','fortnox_access_token_oauth2','fortnox_refresh_token','fortnox_access_token_expiry_time','fortnox_delivery_ways','fortnox_payment_terms','fortnox_price_lists','fortnox_organization_number','fortnox_auth_code'
		];

		foreach( $fortnox_keys as $key ) {
			delete_option( $key );
		}

		self::respond( [ 'message' => __( "Fortnox account has been logged out.", WF_Plugin::TEXTDOMAIN ) ] );
	}
}
