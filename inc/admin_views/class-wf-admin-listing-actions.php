...
use inc\fortnox\api\WF_Delivery_Ways;
use inc\fortnox\api\WF_Invoices;
use inc\fortnox\api\WF_Orders;
use inc\fortnox\api\WF_Payment_Terms;
use inc\fortnox\api\WF_Predefined_Accounts;
use inc\fortnox\api\WF_Price_Lists;
use inc\fortnox\api\WF_Products;
use inc\fortnox\api\WF_Refunds;
use inc\fortnox\api\WF_Request;
use inc\fortnox\WF_Plugin;
use inc\fortnox\WF_Utils;
use inc\help\WF_Help_Links;
...
class WF_Admin_Listing_Actions{
    ...
    public static function manual_sync_order( $order_id ){
        try {
            ...
            if ( in_array( $error->getCode(), [ WF_Orders::FORTNOX_ERROR_CODE_ORDER_ALREADY_INVOICED,  WF_Orders::FORTNOX_ERROR_CODE_ORDER_ALREADY_INVOICED_2,  WF_Orders::FORTNOX_ERROR_CODE_ORDER_ALREADY_INVOICED_3] ) ){
                if( $refund_response = self::maybe_sync_refund( $order_id ) ){
    ...
    private static function get_orders_for_date_range_sync( $from_date, $to_date, $status ) {
	    $query_args = [
		    'limit'         => -1, // No limit on the number of results
		    'status'        => array('wc-' . $status), // Order status
		    'orderby'       => 'date',
		    'order'         => 'ASC',
		    'date_created'  => $from_date . '00:00:00...'. $to_date . '23:59:59', // Date range
	    ];

	    $ids = wp_list_pluck( wc_get_orders( $query_args ), 'ID' );
	    return $ids;
