...
namespace inc\wetail\admin;

use inc\prototypes\WF_Singleton;
...
if ( ! class_exists( __NAMESPACE__ . "\WF_Admin_Settings" ) ):
	class WF_Admin_Settings extends WF_Singleton {
		 ...
		 * @return mixed
		 */
		public static function add_field( $setting, $callback = null ) {
			$settings = self::getInstance();
			$defaults = [
				'page'    => null,
				'tab'     => "general",
		...
		public static function add_custom_field( $setting = [], $html = '' ): void {
			$settings = self::getInstance();
			$defaults = [
				'page'     => null,
				'tab'      => "general",
		...
		public static function display_page() {
			...
			$page_tabs  = $settings->pages[ $_REQUEST[ 'page' ] ][ 'tabs' ];
			$tabs       = array();
			$saveButton = $currentTab[ 'saveButton' ];

			if ( 1 < count( $page_tabs ) ) {
				$hasTabs = true;
			...
			include_once FORTNOX_PLUGIN_PATH . '/inc/templates/settings.php';
		}
	}
endif;
