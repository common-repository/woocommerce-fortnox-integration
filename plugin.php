...
use inc\admin_views\WF_Product_Additional_Fields;
use inc\api\WF_Routes;
use inc\fortnox\api\WF_Orders;
use inc\fortnox\api\WF_Refunds;
use inc\fortnox\WF_Ajax;
use inc\fortnox\WF_Plugin;
use inc\fortnox\WF_Utils;
...
function render_notices(): void {
	echo '<div class="error"><p><strong>' . esc_html__( 'The Fortnox integration for WooCommerce requires WooCommerce to be installed and activated, which is not the case. Hence, the plugin was deactivated.', SLUG ) . '</strong></p></div>';
}
...
add_action( 'plugins_loaded', function () {
	...
	if ( ! is_woo_active() && ! ( defined( 'UNITTEST' ) ) ) {
		add_action( 'admin_notices', 'render_notices' );
		deactivate_plugins( plugin_basename( __FILE__ ) );
	...
	add_action( 'wf_order_after_create_or_update', [ 'inc\fortnox\api\WF_Invoices', "maybe_create_invoice" ], 10, 1 );

	# create credit note on refund
	if ( get_option( 'credit_note_on_refund' ) ) {
		add_action( 'woocommerce_order_refunded', [ 'inc\fortnox\api\WF_Refunds', "handle_refund" ], 10, 2 );
	}
...
add_action( 'init', function () {
	...
	add_action( 'woocommerce_api_create_order', [ 'inc\fortnox\WF_Plugin', "set_sequential_order_number" ], 10, 2 );
	...
	add_filter( 'woocommerce_order_number', [ 'inc\fortnox\WF_Plugin', "get_sequential_order_number" ], 10, 2 );
...
add_action( 'admin_init', function () {
	...
	// Add admin scripts
	add_action( 'admin_enqueue_scripts', [ 'inc\fortnox\WF_Plugin', "add_admin_scripts" ] );

	// Add Fortnox column to class-wf-orders table
	add_filter( 'manage_edit-shop_order_columns', [ 'inc\fortnox\WF_Plugin', "add_orders_table_columns" ] );
	add_filter( 'woocommerce_shop_order_list_table_columns', [ 'inc\fortnox\WF_Plugin', "add_orders_table_columns" ] );
	...
	add_filter( 'manage_edit-product_columns', [ 'inc\fortnox\WF_Plugin', "add_products_table_columns" ] );
	...
	add_action( 'load-post.php', [ 'inc\fortnox\WF_Plugin', "add_meta_boxes" ] );
	add_action( 'load-post-new.php', [ 'inc\fortnox\WF_Plugin', "add_meta_boxes" ] );

	# Sync of the product with Fortnox
	add_action( 'save_post', [ 'inc\fortnox\WF_Plugin', "sync_changes_to_fortnox" ] );
} );
...
add_action( 'admin_init', 'wetail_fortnox_add_styles_to_products_edit_view' );

function wetail_fortnox_add_scripts_for_my_account() {
	add_action( 'wp_enqueue_scripts', [ 'inc\fortnox\WF_Plugin', "add_scripts_for_my_account" ] );
}

function wetail_fortnox_add_styles_to_products_edit_view() {
	add_action( 'admin_print_scripts', [ 'inc\fortnox\WF_Plugin', "add_styles_to_products_edit_view" ] );
}
...
add_action( 'upgrader_process_complete', [ 'inc\WF_Migrate', 'wp_update_completed' ], 10, 2 );
